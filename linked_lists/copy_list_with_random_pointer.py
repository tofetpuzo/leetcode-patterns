...
# n brand new nodes, where each new node has its value set
# to the value of its corresponding original node. Both the next
# and random pointer of the new nodes should
# point to new nodes in the copied list such
# that the pointers in the original list and copied
...
class Solution(object):
    def copy_random_list(self, head):
        ...
        cur = head
        while cur:
            old_to_new_ref[cur] = Node(cur.val)
            cur = cur.next

        # Assign next and random pointers for the new nodes
        cur = head
        while cur:
            new_node_for_deep_copy = old_to_new_ref[cur]
            new_node_for_deep_copy.next = old_to_new_ref.get(cur.next)
            new_node_for_deep_copy.random = old_to_new_ref.get(cur.random)
            cur = cur.next

        return old_to_new_ref[head]
